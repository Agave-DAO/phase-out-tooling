...
contract AgaveTreasuryWithdrawerTest is Test {
    ...
    function test_withdrawAmount(uint256 amount, uint8 i) public {
        ...
        assertLe(owned1, owned2);
        assertGe(available1, available2);
        assertApproxEqAbs(owned2, owned1 + withdrawn, 100);
        assertApproxEqAbs(available2, available1 - withdrawn, 100);
    }

    function test_failWithdrawAmount(uint256 amount, uint8 i) public {
        vm.assume(i < reserves.length - 1);
        uint256 balance1 = IERC20(agTokens[i]).balanceOf(DAO);
        uint256 owned1 = IERC20(reserves[i + 1]).balanceOf(DAO);
        uint256 available1 = IERC20(reserves[i + 1]).balanceOf(agTokens[i]);
        withdrawer.withdrawAssetOnBehalf(reserves[i + 1], agTokens[i], amount);
        uint256 balance2 = IERC20(agTokens[i]).balanceOf(DAO);
        uint256 owned2 = IERC20(reserves[i + 1]).balanceOf(DAO);
        uint256 available2 = IERC20(reserves[i + 1]).balanceOf(agTokens[i]);
        ...
        assertEq(owned1, owned2);
        assertEq(available1, available2);
        assertApproxEqAbs(owned2, owned1 + withdrawn, 100);
        assertApproxEqAbs(available2, available1 - withdrawn, 100);
    }
